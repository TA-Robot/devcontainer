# F04-L-PYBASH-001: execution and analysis plan
...
- C0 depth curve already has first observations.
- C1 may delegate wrapper/CLI contract verification while primary owns persistent store integration.
- CV can execute fresh-process/malformed-state challenges.
- CS can stage implementation then independent lifecycle verification.
...
- Why did all accepted effort levels saturate the current rubric?
- How much extra time/tokens do max/ultra consume without observed quality difference?
- Can split lifecycle criteria reveal partial correctness missed by the broad tests?
