# F02-L-PYBASHJS-001: execution and analysis plan
...
- CP may assign input/persistence, restart/render, and cleanup shards because each has an isolated reproducer; participant width follows shard count and available slots.
- CC compares independent whole-diff reviews for anchoring-resistant recall.
- CV runs reproductions and challenges severity after maker findings.
- CS may stage lifecycle mapping, review, verification, then synthesis; handoff time is explicit.
...
- case timeout safety cap: 105 minutes
...
- Which lifecycle shard dominates wall time?
- Does parallel review improve recall before synthesis cost dominates?
- Does verifier activity remove false positives or merely repeat reproductions?
- Which depth first captures the shared ownership assumption?
