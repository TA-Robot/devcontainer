# F01-L-PYBASHJS-001: execution and analysis plan
...
- C1 can delegate extension-side tracing while primary owns lifecycle synthesis.
- CP can use two independent location shards only if both consume the same immutable boundary schema; reconciliation time is measured.
- CV may verify boundary ownership against source after the maker graph is complete.
...
- case timeout safety cap: 100 minutes
...
- Does sharding reduce navigation time across four runtimes?
- How large is the cross-boundary synthesis tail?
- Do deeper settings improve recovery/ownership coverage?
- How much simulator setup contributes to user wait?
