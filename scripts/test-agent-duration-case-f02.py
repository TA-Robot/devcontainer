...
CATALOG_PATH = ROOT / "experiments" / "multi-agent-duration" / "catalog" / "families" / "f02.json"
...
class F02CodeReviewCaseTests(unittest.TestCase):
    ...
    def test_snapshots_are_reproducible_at_fixed_time(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="duration-f02-repro-") as raw:
            root = Path(raw)
    ...
    def test_review_artifact_cannot_hide_fixture_modification(self) -> None:
        with tempfile.TemporaryDirectory(prefix="duration-f02-integrity-") as raw:
            fixture_dir = Path(raw) / "fixture"
            build_fixture(
                "F02-S-PY-001",
                fixture_dir,
                catalog_path=CATALOG_PATH,
                fixture_id="f02-integrity",
                now=FIXED_CALIBRATION_TIME,
            )
            workspace = fixture_dir / "workspace"
            _install_known_good_for_test("F02-S-PY-001", workspace)
            with (workspace / "workspace.py").open("a", encoding="utf-8") as handle:
                handle.write("\n# unauthorized review-time edit\n")
            result = evaluate_fixture(fixture_dir)
            self.assertEqual(result["status"], "fail")
            self.assertIn("review-seeded-recall", result["score"]["failed_check_ids"])
