...
class DocumentationRunbookFamilyTests(unittest.TestCase):
    ...
    def test_fixture_snapshots_are_reproducible(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="duration-f07-repeat-") as raw:
            root = Path(raw)
    ...
    def test_source_runtime_and_public_validator_tampering_is_rejected(self) -> None:
        protected_paths = {
            "F07-S-MD-001": "agentctl.py",
            "F07-M-MDBASH-001": "tools/check_runbook.py",
            "F07-L-MDPYBASH-001": "src/backend.py",
        }
        fixed_time = datetime(2026, 8, 27, 12, 0, tzinfo=timezone.utc)
        with tempfile.TemporaryDirectory(prefix="duration-f07-integrity-") as raw:
            root = Path(raw)
