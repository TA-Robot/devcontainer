...
`process.log` remains owner-only raw evidence and is not rewritten into a redacted transcript. Once a provider process closes, agentctl atomically retains at most its final 8 MiB and writes owner-only `log-retention.json` containing the original size, retained size, and whether an initial partial line was discarded. This bounds completed provider logs while preserving the final structured result area. A running provider may temporarily exceed 8 MiB before terminal retention runs; runner and supervisor logs do not yet have the same rotation policy.
...
Version 0.7 provides terminal-disconnect continuity, deterministic recovery, bounded local scheduling, collision-resistant execution namespaces, immutable integration handoff reports, bounded operational log views, completed-provider tail retention, read-only cleanup eligibility, and symmetric Codex / Claude / Grok dispatch. It does not claim that a process survives a container or host restart, that a same-UID process is a security boundary, that path overlap predicts all semantic conflicts, that best-effort redaction removes every secret, that a live raw log is already size-bounded, or that a port probe reserves the operating system socket for the provider. Verified job-scoped Docker teardown, destructive worktree/branch GC, evidence expiry, and runner/supervisor log rotation remain unimplemented. Until those land, treat `gc --dry-run` as evidence for a primary-owned manual decision, never as authorization for broad deletion.
