...
このdirectoryは、VS Code companion extensionで使うミラのidentity anchorとpixel-art framesを提供します。extension本体は`extensions/mira-companion/`、Codex / Claude / Grok / agentctl activity bridgeの互換entrypointは`scripts/mira-codex-hook.py`、連携contractは`docs/mira/architecture.md`を正本とします。
