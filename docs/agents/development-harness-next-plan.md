...
**Cycle 005でCLI同期の限定課題を固定し、新方式のA/B・評価・統合まで完了した。**
[結果](../../experiments/development-harness/cycle-005/result.md): 両方13/13、A 587.194秒・B 780.894秒。
追加レビューの互換性を根拠にBのruntimeを採用したが、追加feedback修正は発動せず、既定化は見送る。
2026-09-06のscope訂正: 外部projectの提供は不要。候補一つの棄却を外部案件の必須化へ広げない。
[題材選定時の候補](../../experiments/development-harness/selection/cli-sync-candidate.md)から、限定課題として具体化した。
P1の[実行contract](../../experiments/development-harness/feedback/README.md)と
[検証証跡](../../experiments/development-harness/feedback/validation.json)を追加した。
[P0の実装と検証](../../experiments/development-harness/terminal/README.md)を土台に、
正常に検証できる共通環境で、自由な自己検証と自動的な検証フィードバックを比較する。
当初の大規模先行案は候補監査で見直し、実際に再現した小規模同期課題を先に評価した。
大規模の継続開発へ拡張したと偽らず、新方式のその範囲は未測定として残す。
...
`terminal/pair.py`で「実行 → 正常提出または期限／異常 → 停止確認 → 保全 → 固定検査 → 集計」を接続した。
旧`automatic/finalize_interrupted.py`の停止・保全・固定検査の境界を維持し、archive/observerを再利用する。
旧時計の予約投影へv2 stateを流さず、旧finalizerと原記録は保持する。
...
第一候補だった**既存の依存jobを、独立検証・中断復旧・収集reportまで有限の操作で進める利用経路**は、
[不足監査](../../experiments/development-harness/selection/README.md)で中核が既存CLIの合成で成立したため棄却した。
以下は選定時の要求候補と判定条件を保持したもの。新しい大規模課題として採用済みではない。
...
監査では上記機能の多くが既にあることを再現した。検証後の再検査失敗が後続dispatchを止めない不足は
残るが、その修正だけで大規模の継続変更を代表するとは判断しない。
...
このrepoで大規模の条件を満たせなければ、大規模への有効性は未測定として残す。
それを外部projectの提出待ちへ変えず、この基盤内の改善と、その範囲に合う評価を進める。
...
| P1（完了）: 比較する手順を固定 | 公開check選択規則、A/B差分、検査・再開費込みの時計、image・起動設定・read-only入力の同一性を実装 | 実Codex sandboxと疑似providerで能力・修正・停止を確認。次の課題別check表と上限はP2で確定 |
| P2（小規模完了）: 課題と評価器を固定 | Cycle 005の要求・13項目・全上限を固定。大規模側は未固定 | 対象source・利用要求・oracle識別能力・規模条件・全上限が未確定なら開始しない |
| P3（小規模完了）: 最小の探索比較 | Cycle 005の小規模A/Bを実行・報告。大規模側は未実施 | 全条件・未着手・中断を台帳に残し、自動評価。基盤の重大な故障なら未開始のcellを止め、原因修理へ戻す |
| P4（Cycle 005完了）: 採否と確認 | runtimeはBを採用、feedback既定化は見送り。効果の向きから条件を変えない | 有望なら別の未見taskで確認。効果なしならその介入を終了し、勝つまでrunを追加しない |
...
以下の順序図と初期の費用枠は当初案を保持する。Cycle 005は課題監査後に小規模先行へ変更し、
実際の順序・上限・結果は同cycleの固定protocolと結果を正本とする。未実施の大規模runを予約済みとはしない。
...
| 大きそうな新機能を発明しているのでは | 大規模候補の不足はまだ要監査。既存scriptで足りるなら機能を膨らませず、この基盤内の別の不足を選ぶ。規模は実要求に合わせる |
...
P0は実Dockerを含む新版27テストと旧ハーネス42テストで確認した。
P1は同じ公開情報・named permission profile・有限修正を実装し、P0と旧suiteも再確認した。
P2第一候補は11項目の監査で棄却し、Cycle 005のCLI同期を小規模として比較・統合した。外部projectの指定は不要。
新課題・adapterを含む開始前96テストが合格し、統合後の実imageと稼働中containerでも確認した。
停止所有者自身のSIGKILLやdaemon喪失後の無人再起動は提供せず、停止の不明を保持する。
ユーザーへの追加の採点・日報・再承認依頼は設けない。
