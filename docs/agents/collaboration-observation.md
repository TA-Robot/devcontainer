...
Codexのshell surfaceは`exec_command`も正規化対象です。test commandの前に
`PYTHONDONTWRITEBYTECODE=1`などのenvironment assignmentがある場合もtestとして
分類しますが、command本文とoutput本文は保存しません。
...
skill wrapperをskill directoryから起動しても、nearest ancestorの
`.agent/config.json`をcopied project rootとして解決してからworkspace keyを作ります。
このmarkerがない環境だけprocess current directoryへfallbackします。
