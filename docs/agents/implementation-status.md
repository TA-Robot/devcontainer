...
- Codex / Claude / Grok hooks and sanitized `agentctl` lifecycle envelopes automatically summarize solo, delegated, and managed-job episodes without human input. The bounded local ledger survives rebuilds in a dedicated named volume and records timing, worker topology, structured test outcomes, rework proxies, post-worker-tail proxies, and hook coverage while excluding content and raw identifiers.
- Semantic mechanism, constraint, relation, lifecycle, quality, and actual human-review time remain explicitly unknown when no reliable machine evidence exists; topology and terminal success are not used as substitutes.
- The Dev Container lock validator requires the dedicated observation volume and configured episode directory; container smoke recreates a container against one volume and verifies the ledger survives without leaking probe content.
