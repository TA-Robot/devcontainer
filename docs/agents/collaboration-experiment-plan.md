...
更新: 2026-09-06。状態: **比較設計と既存候補の一次監査。live protocolは未seal。**
...
次はその資産を使った別版の準備を優先し、校正不成立・実利用との不一致・過大な準備費で再考する。
...
まず相談比較の一単位を、以下まで具体化する。既存F03-Lのコピーをそのままliveへ出さず、
原因そのものを開示しない別版を設計・校正する。要求、出力形式、観測手段は両条件へ等しく公開する。
既存case/revisionを無言で変更しない。全項目が固定されるまでliveを開始しない。
