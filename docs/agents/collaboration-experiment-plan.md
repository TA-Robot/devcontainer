...
既存の実測を通常配布入口へ接続し、観測と限界を取り出せる利用確認を先行する。
次の比較はCycle 002の具体的な未達要求と協働の作用を対応させて選ぶ。
Cycle 003のtemplate評価と別題材の継続開発runnerは分けて接続費を確認する。
要求・公開検証・外部判定・接続差分から一件の限定実装か保留かを決めたら選定を終了する。
方式・新要求・live予算はその後に固定し、配布だけで改良前後の再評価まで済んだとは扱わない。
