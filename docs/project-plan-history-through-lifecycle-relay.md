...
更新: 2026-09-07。情報介入の初回比較と、3者の独立批判に基づく優先順位の修正を反映。
...
2026-09-06のrepository監査と2026-09-07までの実比較。過去の未実施扱いへの巻き戻しも、実装済みから効果実証済みへの飛躍もしない。
...
queueの限定した範囲では、**実測を案内へ戻し、live未使用だった初期状態で再比較する一循環を実施した**。
結果は独立確認候補であり、方式全般・別family・継続開発での改善効果は未確認。
...
| M2: 協働比較の自動実行 | 最初に適格となった方式を既存terminal/所有権/評価へ接続。受渡し、全参加者停止・全usage・統合・外部評価を疑似providerと実Dockerで確認 | queue reviewに加え、[自力調査→任意の相談→最終化](../experiments/development-harness/queue_review/conditional_v1/README.md)を実装。修正済みcodeと質問の受渡し・両分岐・異常停止を実Dockerで確認。liveでは両側ともsubmit。相談後はfresh makerで、同じsessionの継続は未対応 |
...
| M4: ハーネス改良と再評価 | 採用/条件限定/棄却/保留を決め、指示・情報・実行制御を版付きで改修。結果を見ていない課題で改良前後を再評価 | **限定した初回循環を実施**。同じ相談能力へ情報だけ追加し、留保条件で[1組を比較](../experiments/development-harness/queue_review/conditional_v1/result.md)。両者submit・4/4、情報ありは時間10.176%減/input13.615%減。僅差の1組なので独立確認候補。一般既定化・広い課題での再現性は未達 |
...
[条件付き相談の情報介入](../experiments/development-harness/queue_review/conditional_v1/result.md)では、
両条件に同じsubmit/consult能力を与え、一方だけに前回の範囲付き観測と起動案内を追加した。
6テストで両分岐・修正済みcodeの引継ぎ・異常停止を実Dockerで確認し、留保していたconfirmationで1組を実行。
両側とも相談せず4/4、条件時間95.209秒対85.520秒。input/outputも減ったが、事前の時間基準を約0.168秒だけ満たした。
相談選択の変化は観測せず、案内は独立確認候補に留める。今回のconfirmationはlive使用済みとして保持する。
...
分類一致は根拠の意味・全品質・協働効果ではない。
[有限relayによる実比較](../experiments/development-harness/selection/relay_v1/result.md)で、
agentの公開probe要求・結果受取り・最終提出・自動照合まで接続し、単独と固定事前助言の1組を完了した。
agent自身にも完成実装を持たせず、実CLIのtoolsなし要求と疑似provider経路を7テストで確認した。
両条件4分類一致・採用保留、132.980秒対195.559秒、相談側のinput/outputは約38%増。
この範囲で固定事前相談を既定に追加しない。1組の割当は使用済みで、同課題の追加実行はしない。
この一件で選定監査・relay接続を閉じ、3段階全再演や全方式runnerへ広げない。
結果への独立批判を受け、現行CLIに未達が残るかも確認した。採用permission・root/二重slash境界の既存3テストが通り、
旧候補の欠陥は修正済みなので再修正しない。次の固定事前相談を作る理由にはしない。
...
- **強い対照:** soloにも自己検証、複数案、試作、同じ公開toolを許す。追加agent分の資源と、
  別context/独立性の効果を分離する問いには、それに対応する対照を設ける。
  同時間のワークフロー比較では全消費を併記し、各作用を分離したとは主張しない。同じoutput上限だけを同費用と呼ばない。
