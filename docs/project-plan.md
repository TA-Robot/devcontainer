...
段階実行・snapshot・累積計測・外部のCLI検証を実装し、fake providerと実Dockerによる18の事前テストを完了した。大規模の対照条件を開始しており、まだ比較結果や改善の採否はない。小規模への同一介入の再評価も未完了。一般的な速度効果を問う段階では複数task・複数run・実行順の交替を事前に定める。既存testの起動・時間依存の不安定さは残件として追跡し、成功runだけで信頼性を判断しない。
