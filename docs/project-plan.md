...
**次の着手点はP2: 実際に進めたいprojectと変更要求から大規模側の別候補を選ぶ。**
...
| P2（継続） | 第一候補を監査・棄却。実案件の別候補から課題・oracle・予算を固定 | 実需要・依存・退行を説明でき、正しい成果と代表的欠陥を自動で識別。対象と要求が未定の間はsealしない |
...
実装を膨らませず、実際に必要なprojectの変更系列へ切り替える。対象sourceと利用要求が未確定の間は
大規模効果を未測定とし、小規模の成功で代用しない。
