...
| M1: 実験群の適格性 | 既存候補を監査し、方式に適する課題と自動評価の範囲を決定。正解の多様性・欠陥検出・solo対照・費用計測を確認し、初回protocolを固定 | **進行中**。既存4課題の校正に加え、診断別版を実装・20候補で校正。非agent解法で解けたため主課題には不採用。主比較とlive上限は未固定 |
| M2: 協働比較の自動実行 | 最初に適格となった方式を既存terminal/所有権/評価へ接続。受渡し、全参加者停止・全usage・統合・外部評価を疑似providerと実Dockerで確認 | [直列の相談→maker経路](../experiments/development-harness/consultation/README.md)は実装・疑似provider/実Docker検証済み。live adapter・能力probe・環境同等性の確認は未完了 |
| M3: 方式別の探索比較 | 相談・review・案収集・実装比較を対象に、適格な課題で有限の比較。正/負/不明、発動状況、失敗、全費用、未着手を保存 | 一方式の結果で他方式を済ませない。順序はM1の識別力と準備費で決める |
| M4: ハーネス改良と再評価 | 採用/条件限定/棄却/保留を決め、指示・情報・実行制御を版付きで改修。結果を見ていない課題で改良前後を再評価 | **ここまでを最初の改善循環の完了条件**とする。不採用・保留も有効 |
...
次はこの限定した問いの介入・採否基準を固定し、live adapterと能力・環境の確認を接続する。
