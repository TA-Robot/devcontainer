...
1. [課題選定メモ](../experiments/development-harness/multi-scale/task-selection.md)から実需要のある大規模taskを確定する。現在の第一候補は、別projectへの導入・既存設定を保持する継続更新・中断復旧までを一つの利用経路として完成させる開発。機能の採用や大規模への分類はまだ確定していない。
...
この改訂で小規模・大規模の評価設計を追加したが、新しい大規模runはまだ実施していない。一般的な速度効果を問う段階では複数task・複数run・実行順の交替を事前に定める。既存testの起動・時間依存の不安定さは残件として追跡し、成功runだけで信頼性を判断しない。
