...
更新: 2026-09-07。ユーザーによる目的の再確認と、情報介入の初回比較を反映。
...
次のplanning priorは、発見用1組を採否集計から外し、両条件順序を扱う2組の固定確認。
同じ入力を使う場合は反復と明示し、新protocolで全予算・採否・停止を固定する。
2組は順序の偏りを点検する最小coverageで、有意差や一般化の保証ではない。結果に応じた追加実行は行わない。
