...
| M4: ハーネス改良と再評価 | 採用/条件限定/棄却/保留を決め、指示・情報・実行制御を版付きで改修。結果を見ていない課題で改良前後を再評価 | 集計に校正のvalidity gateを追加し、実測ガイドへ採否を反映。新しい評価版・確認課題での再評価は未実施。**ここまでを最初の改善循環の完了条件**とする |
...
次は公開された構造化scope・根拠関係による別評価版を校正する。同じ旧oracleで本数を増やさない。
