...
| M1: 実験群の適格性 | 既存候補を監査し、方式に適する課題と自動評価の範囲を決定。正解の多様性・欠陥検出・solo対照・費用計測を確認し、初回protocolを固定 | **着手済み**。候補4課題の既存校正は4/4合格。公開情報などの適用限界を発見。新比較版の校正・live上限は未固定 |
| M2: 協働比較の自動実行 | 最初に適格となった方式を既存terminal/所有権/評価へ接続。受渡し、全参加者停止・全usage・統合・外部評価を疑似providerと実Dockerで確認 | 未実装部分を限定して着手。汎用engineの完成を待たない |
...
一次監査でF03-Lは公開文が原因のevent順序を明示すると確認した。現版を原因発見の相談比較へ
そのまま採用しない。最初の具体作業は、この公開情報と評価対象を分離する別版の設計・校正。
既存版は再現手順の構築を測る資産として残す。他候補も現contractで答えられる問いに限定する。
