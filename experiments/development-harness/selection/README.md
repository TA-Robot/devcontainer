...
2026-09-06のscope訂正: この棄却から外部projectの提出を必須とした判断は撤回した。
次は[このdevcontainer基盤のCLI同期・失敗復旧](cli-sync-candidate.md)を題材に進める。
原監査JSONは当時の観測・判断として保持し、外部project指定待ちを現在の要件にしない。
...
| 次の分岐 | このdevcontainer基盤自身のCLI同期・失敗復旧を具体化する |

ForgeRoom/Evidence Forge/DevRelayの既存実験を新しい未見の実案件へ読み替えない。
この基盤の実利用の要求、現状の不足、前段階が作る状態を後続変更で引き継ぐ必要を確認してから、
課題・公開check・oracle・予算・順序を固定する。外部projectは現在の作業に必要ない。
