...
def main():
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if len(requests)<=3:
                if len(requests)==1:
                    command = "echo 'print(dict())' > /work/broken.py && python3 /public/public_check.py --candidate /work/broken.py --output /work/broken-report.json"
                elif len(requests)==2:
                    command = "cp /public/fifo.py /work/submission.py && python3 /public/public_check.py --candidate /work/submission.py --output /work/repaired-report.json"
                else:
            ...
            events=[{'type':'response.created','response':{'id':'resp_test'}},
