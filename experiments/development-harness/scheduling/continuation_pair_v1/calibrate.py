...
def public_execution(cohort, public, output, name):
    ...
    container = 'continuation-public-'+uuid.uuid4().hex
    ...
    try:
        ...
        result['removed'] = subprocess.run(['docker', 'rm', '-f', container], capture_output=True, timeout=15).returncode == 0
...
def execute(output):
    ...
    assess = cohort.load('continuation_calibration_assess', HERE/'assessment.py')
