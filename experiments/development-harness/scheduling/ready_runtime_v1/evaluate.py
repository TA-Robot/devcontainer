...
def evaluate(candidate, scenarios, output):
    ...
    identity = {name: digest((HERE/name).read_bytes()) for name in ('runtime.py', 'transport.py', 'evaluate.py', 'TASK.md', 'launcher.py', 'recovery.py')}
    ...
    report = {'kind': 'scheduling-runtime-ready-v1', 'status': 'running', 'cases': [],
    ...
    try:
        for case in scenarios:
            ...
            try:
                ...
                row.update(status='unmeasured', failure=type(error).__name__, failure_detail=str(error))
            finally:
