...
def main():
    ...
    result = {'status': 'withhold', 'reason': None, 'usage': None}
    process = None
    total = 0
    handlers = {}
    ...
    try:
        ...
        try:
            ...
            result['accounting'] = collect(output / 'sessions.private', output / 'inventory.sqlite')
            accounting = result['accounting']
            if accounting['status'] != 'completed' and result['reason'] is None:
                result['reason'] = 'incomplete participant accounting'
            if (result.get('returncode') == 0 and result['reason'] is None
                    and accounting['status'] == 'completed'):
                amount = accounting['usage']
                if amount['output_tokens'] > limits['output_tokens']:
                    result['reason'] = 'observed output token cap exceeded'
                elif limits['condition'] == 'solo' and len(accounting['threads']) != 1:
                    result['reason'] = 'unexpected solo participant'
                else:
                    result['status'] = 'completed'
                    result['usage'] = amount
        except (OSError, ValueError, sqlite3.Error) as exc:
            result['reason'] = 'collection: ' + type(exc).__name__
