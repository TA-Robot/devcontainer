"""Single-use old/new/solo cohort: frozen source, all-submission barrier, grading."""
import argparse
import hashlib
...
PROMPTS = {'standard': SCHEDULING/'adaptive_pair_v1/adaptive.md',
           'refined': SCHEDULING/'failure_refinement_v1/adaptive.md',
           'solo': SCHEDULING/'adaptive_pair_v1/solo.md'}
...
def identity():
    ...
    files = list(HERE.glob('*.py')) + [HERE/'config.json', HERE/'protocol.md',
             ...
             ROOT/'scripts/test-refinement-cohort.py', ROOT/'scripts/report-dynamic-solo-profile.py']
...
def prepare_inputs(output, fake):
    ...
    populations = load('cohort_populations', HERE/'populations.py').generate(read(HERE/'config.json'))
    ...
    try:
        ...
        for name in config['conditions']:
            ...
            save(output/('start-'+name+'.json'), {'condition': name, 'ordinal': len(result['actors'])+1,
                                                 'prompt_sha256': sha(PROMPTS[name])})
            ...
            if fake and name == 'refined':
                if fake_case == 'missing': mode = 'missing_usage'
                elif fake_case == 'timeout': mode, seconds = 'child_timeout', 8
                elif fake_case == 'cost': cap = 40
            row = actor.run(output/'actors'/name, output/'public', condition='solo' if name == 'solo' else 'adaptive',
                            prompt=PROMPTS[name].read_text(), fake=fake, fake_mode=mode,
                            seconds=seconds, output_tokens=cap, auth=auth)
...
def run(output, *, fake=False, auth=None, fake_case='normal'):
    ...
    if not fake:
        ...
        if (calibrated['status'] != 'passed' or calibrated['source_sha256'] != initial
                or calibrated['public_cases_measured'] != 24 or not calibrated['all_containers_removed']):
