"""Fixed notification fixture: delayed usage versus deliberately omitted usage."""
import http.server
import itertools
import json
...
def main():
    ...
    control = mode == 'notice_control'
    missing = mode == 'notice_missing'
        ...
        def do_POST(self):
            ...
            if root and step==1:
                message={'id':'tool_'+str(number),'type':'custom_tool_call','call_id':'call_'+str(number),
                    ...
                    'input':'text(await tools.exec_command({cmd:"cp /public/fifo.py /work/submission.py"}));'}
            elif root and step==2 and control:
                message={'id':'tool_'+str(number),'type':'custom_tool_call','call_id':'call_'+str(number),
