...
Codex lifecycle hookのsanitized eventを、VS Code下部の短いpixel-art世界へ変換するworkspace extensionです。コーディングが主役で、ミラはその進行に合わせて世界の中を動きます。

- read / planning / edit / shell / test / delegationごとに対応するmap拠点へ自動移動
- idle中は資料庫、作戦卓、工房、test gate、dispatch dockの間を時々散歩
- subagent稼働中は小さな仲間spriteがdockへ出現
...
- prompt、code、tool arguments、command、tool output、transcriptは読み込みも保存もしない
